...
const Comparison = () => {
  ...
  // 🔹 Dynamic headings & intro content (reused from AllCasinoDetail for consistency)
  const pageContent = {
    '/': {
 ...
 const casinoData = [
  ...
  // Map URL paths to tags (reused from AllCasinoDetail)
  const pathToTagMap = {
    '/': 'main',
  ...
  const defaultContent = {
    heading: 'Casino Comparison (Default)',
    intro1: 'Compare the best crypto casinos available. This is the default content, shown when no specific route matches.',
    intro2: '',
  ...
  return (
    <div>
      <section className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
        <h1 className="text-2xl sm:text-3xl md:text-4xl font-semibold mb-4 sm:mb-6 leading-tight text-gray-900">Comparison: {heading}</h1>
        ...
        {filteredCasinos.length > 0 ? (
            ...
            <table className="min-w-full bg-white text-xs">
              <thead>
                <tr className="bg-gray-200 border-b border-gray-300">
                  <th className="px-2 py-1 border-r border-gray-300 font-semibold">Casino</th>
                  <th className="px-2 py-1 border-r border-gray-300 font-semibold">Desc.</th>
                  <th className="px-2 py-1 border-r border-gray-300 font-semibold">Security</th>
                  <th className="px-2 py-1 border-r border-gray-300 font-semibold">Pros</th>
                  <th className="px-2 py-1 border-r border-gray-300 font-semibold">Cons</th>
                  <th className="px-2 py-1 border-r border-gray-300 font-semibold">Signup</th>
                 
                  <th className="px-2 py-1 border-r border-gray-300 font-semibold">Bonuses</th>
                  
                  <th className="px-2 py-1 font-semibold">Supported coins</th>
                </tr>
              </thead>
              <tbody>
                {filteredCasinos.map((casino, index) => (
                  <tr key={index} className={`${index % 2 === 0 ? 'bg-gray-50' : 'bg-white'} border-b border-gray-300`}>
                    <td className="px-2 py-1 border-r border-gray-300 font-medium max-w-xs truncate">{casino.name}</td>
                    <td className="px-2 py-1 border-r border-gray-300 max-w-xs truncate">{casino.description}</td>
                    <td className="px-2 py-1 border-r border-gray-300">{casino.security}</td>
                    <td className="px-2 py-1 border-r border-gray-300">
                      <ul className="list-disc ml-2 space-y-0.5">
                    ...
                    <td className="px-2 py-1 border-r border-gray-300 max-w-xs break-words">{casino.signup_process}</td>
                   
                    <td className="px-2 py-1 border-r border-gray-300 max-w-xs break-words">{casino.bonuses_and_promotions}</td>
                  
                    <td className="px-2 py-1 max-w-xs truncate">{casino.supported_coins.join(', ')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <p className="text-base sm:text-lg text-gray-700">No casinos found for comparison in this category.</p>
        )}
